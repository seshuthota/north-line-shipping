...
function TrackingResult({ record }: { record: ShipmentRecord }) {
  return <div className="tracking-result">
    ...
    <div className="result-body"><div><h3>Shipment journey</h3><div className="timeline">{record.events.map((event, index) => <div className={`timeline-item ${event.complete ? 'complete' : ''}`} key={`${event.title}-${index}`}><span className="timeline-dot">{event.complete && <Check />}</span><div><strong>{event.title}</strong><p>{event.location}</p><small>{event.time}</small></div></div>)}</div></div><aside><div><small>SERVICE</small><strong>{record.service}</strong></div><div><small>REFERENCE</small><strong>{record.reference}</strong></div><div><small>RECIPIENT</small><strong>{record.recipient}</strong></div><button className="button button-outline"><Mail /> Get status updates</button></aside></div>
  </div>;
}
...
const legalCopy: Record<string, { title: string; intro: string; sections: [string, string][] }> = {
  privacy: { title: 'Privacy policy', intro: 'How personal information is handled when you use this redesigned demonstration.', sections: [['Information we use', 'This concept stores only demo account preferences and sample activity in your browser. It does not transmit personal data to Northline or any third party.'], ['Local storage', 'You can clear saved demo information at any time from the account dashboard or through your browser settings.'], ['Official services', 'When following a link to an official Northline or OrbitLink service, that destination’s privacy policy applies.']] },
